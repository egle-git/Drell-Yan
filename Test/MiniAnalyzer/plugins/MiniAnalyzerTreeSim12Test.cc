...
class MiniAnalyzerTreeSim12Test : public edm::one::EDAnalyzer<edm::one::SharedResources> {
    public:
        explicit MiniAnalyzerTreeSim12Test(const edm::ParameterSet&);
        ~MiniAnalyzerTreeSim12Test();
    ...
    private:
        ...
        float isoSum, isoSumCorr, relIso, muon_event_weight, muon_norm_weight, tau_event_weight, tau_norm_weight;
...
MiniAnalyzerTreeSim12Test::MiniAnalyzerTreeSim12Test(const edm::ParameterSet& iConfig):
...
MiniAnalyzerTreeSim12Test::~MiniAnalyzerTreeSim12Test()
{
...
void
MiniAnalyzerTreeSim12Test::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
void 
MiniAnalyzerTreeSim12Test::beginJob()
{
    ...
    tree_Z = new TTree("Z", "");
    tree_tauZ = new TTree("TauZ", "");
    tree_info = new TTree("Info", "");
    ...
    tree_Z->Branch("Z_pt", &Z_pt);
    tree_Z->Branch("Z_eta", &Z_eta);
    tree_Z->Branch("Z_phi", &Z_phi);
    tree_Z->Branch("Z_energy", &Z_energy);
    tree_Z->Branch("Z_mass", &Z_mass);
    tree_Z->Branch("Z_px", &Z_px);
    tree_Z->Branch("Z_py", &Z_py);
    tree_Z->Branch("Z_pz", &Z_pz);

    tree_tauZ->Branch("tauZ_pt", &tauZ_pt);
    tree_tauZ->Branch("tauZ_eta", &tauZ_eta);
    tree_tauZ->Branch("tauZ_phi", &tauZ_phi);
    tree_tauZ->Branch("tauZ_energy", &tauZ_energy);
    tree_tauZ->Branch("tauZ_mass", &tauZ_mass);
    tree_tauZ->Branch("tauZ_px", &tauZ_px);
    tree_tauZ->Branch("tauZ_py", &tauZ_py);
    tree_tauZ->Branch("tauZ_pz", &tauZ_pz);

    tree_info->Branch("isoSum", &isoSum);
    tree_info->Branch("isoSumCorr", &isoSumCorr);
    tree_info->Branch("relIso", &relIso);
...
void 
MiniAnalyzerTreeSim12Test::endJob() 
{
    fs->cd();
    tree_muon->Write();
    tree_tau->Write();
...
void
MiniAnalyzerTreeSim12Test::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(MiniAnalyzerTreeSim12Test);
